...
\end{array}
\]
\caption{System of linear equations in $\mathbb{F}^3_2$ induced by Figure~\ref{fig:csc592_bigdata_nubff}. Each $A[i]$ is the $i$-th entry in the backing array 
$A$ (zero-indexed). The keys are $\mathcolorbox{lightorange}{x}$, $\mathcolorbox{lightred}{y}$, $\mathcolorbox{lightpink}{w}$ and $\mathcolorbox{lightpurple}{z}$ 
...
respectively. Array entries and fingerprints are marked with corresponding key color(s). The phantom entry is colored gray. There is one equation for each key. }
